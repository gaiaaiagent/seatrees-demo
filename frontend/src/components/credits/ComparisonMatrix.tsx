      ...
      <div className="flex gap-4 min-w-max lg:grid lg:grid-cols-5 lg:min-w-0">
        {frameworks.map((fw) => (
          <div
            key={fw.id}
            className={cn(
              'glass-card rounded-xl p-5 flex flex-col gap-4 shrink-0',
              fw.highlight
                ? 'border-primary ring-1 ring-primary/20 w-64 lg:w-auto'
                : 'border-border w-56 lg:w-auto'
            )}
          >
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold leading-tight">{fw.framework_name}</h3>
              {fw.highlight && (
